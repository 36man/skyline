...
/**
 * @author lijian
 * @since time: 2022-09-06 13:48
 */
public interface ShortcutConfigurable {
    static String normalizeKey(String key, int entryIdx, ShortcutConfigurable argHints, Map<String, Object> args) {
    ...
    static Object getValue(SpelExpressionParser parser, BeanFactory beanFactory, Object entryValue) {
        Object result;
        String v = String.valueOf(entryValue);
        if (v != null) {
            v = v.trim();
        }
        if (v != null && v.startsWith("#{") && v.endsWith("}")) {
            ...
            Expression expression = parser.parseExpression(String.valueOf(entryValue), new TemplateParserContext());
    ...
    enum ShortcutType {
        ...
        DEFAULT {
            @Override
            public Map<String, Object> normalize(Map<String, Object> args, ShortcutConfigurable shortcutConf,
        ...
        GATHER_LIST_TAIL_FLAG {
            ...
            public Map<String, Object> normalize(Map<String, Object> args, ShortcutConfigurable shortcutConf,
                                                 SpelExpressionParser parser, BeanFactory beanFactory) {
                Map<String, Object> map = new HashMap<>();
                // field order should be of size 2
                List<String> fieldOrder = shortcutConf.shortcutFieldOrder();
                ...
                List<String> values = args.values().stream().map(String::valueOf).collect(Collectors.toList());
                if (!values.isEmpty()) {
        ...
        public abstract Map<String, Object> normalize(Map<String, Object> args, ShortcutConfigurable shortcutConf,
                                                      SpelExpressionParser parser, BeanFactory beanFactory);
    ...
    class GatewayEvaluationContext implements EvaluationContext {
        ...
        public static final String SKYLINE_RESTRICTIVE_PROPERTY_ACCESSOR_ENABLED = "skyline.restrictive-property-accessor.enabled";
        ...
        public GatewayEvaluationContext(BeanFactory beanFactory) {
            ...
            boolean restrictive = env.getProperty(SKYLINE_RESTRICTIVE_PROPERTY_ACCESSOR_ENABLED,
                    Boolean.class, true);
            if (restrictive) {
